...
public class AuthorController {
    ...
    @PutMapping
    public ResponseEntity<AuthorResponse> updateAuthor(@Valid @RequestBody AuthorUpdateRequest request) {
    ...
    public ResponseEntity<Void> addAuthorToBook(@PathVariable UUID authorId, @PathVariable UUID bookId) {
        authorService.addAuthorToBook(bookId, authorId);
        return ResponseEntity.status(HttpStatus.CREATED).build();
    }

    @DeleteMapping("/{authorId}/books/{bookId}")
    public ResponseEntity<Void> removeAuthorFromBook(@PathVariable UUID authorId, @PathVariable UUID bookId) { 
