...
class AuthorControllerTest {
    ...
    private final AuthorCreateRequest createRequest = new AuthorCreateRequest("Jane", "Smith", Sexe.F, null);
    private final AuthorUpdateRequest updateRequest = new AuthorUpdateRequest(authorId, "John", "DoeUpdated");
    ...
    void testSearchAuthorsByLastName() throws Exception {
        ...
        mockMvc.perform(get("/api/authors?lastName=Doe"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(1)));
    }
    ...
    void testAddAuthorToBook() throws Exception {
        ...
        mockMvc.perform(post("/api/authors/{authorId}/books/{bookId}", authorId, bookId))
                .andExpect(status().isCreated());
    }
    ...
    void testAddAuthorToBook_BookNotFound() throws Exception {
        doThrow(new ResponseStatusException(HttpStatus.NOT_FOUND, "Book not found"))
                .when(authorService).addAuthorToBook(bookId, authorId);
    ...
    void testRemoveAuthorFromBook() throws Exception {
        ...
        mockMvc.perform(delete("/api/authors/{authorId}/books/{bookId}", authorId, bookId))
                .andExpect(status().isNoContent());
    }
    ...
    void testGetAuthorsByBook() throws Exception {
        ...
        mockMvc.perform(get("/api/authors/books/{bookId}/authors", bookId))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(1)));
    }
