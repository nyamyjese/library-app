...
class GenreControllerTest {
    ...
    private final GenreUpdateRequest updateRequest = new GenreUpdateRequest(genreId, "Fantasy Updated");
    ...
    void testGetGenreById() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/genres/{genreId}", genreId))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.name").value("Fantasy"));
    }
    ...
    void testSearchGenresByName() throws Exception {
        ...
        mockMvc.perform(get("/api/genres?name=Fantasy"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(1)));
    }
    ...
    void testAddGenreToBook() throws Exception {
        ...
        mockMvc.perform(post("/api/genres/{genreId}/books/{bookId}", genreId, bookId))
                .andExpect(status().isCreated());
    }
    ...
    void testAddGenreToBook_BookNotFound() throws Exception {
        doThrow(new ResponseStatusException(HttpStatus.NOT_FOUND, "Book not found"))
                .when(genreService).addGenreToBook(bookId, genreId);
    ...
    void testRemoveGenreFromBook() throws Exception {
        ...
        mockMvc.perform(delete("/api/genres/{genreId}/books/{bookId}", genreId, bookId))
                .andExpect(status().isNoContent());
    }
    ...
    void testGetGenresByBook() throws Exception {
        ...
        mockMvc.perform(get("/api/genres/books/{bookId}/genres", bookId))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(1)));
    }
    ...
    void testGetBooksByGenre() throws Exception {
        ...
        mockMvc.perform(get("/api/genres/{genreId}/books", genreId))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(1)))
